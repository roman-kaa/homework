...
После проверки через curl контейнер был остановлен, а docker-compose.yml был изменен добавлен  параметр cluster.name=elk-random-. 
...
Filebeat считывал тот же access-лог Nginx и отправлял события в индекс nginx-filebeat-*.
